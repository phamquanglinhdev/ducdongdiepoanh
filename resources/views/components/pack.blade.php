    ...
    @foreach($packs as $pack)
        ...
        <div class="d-flex justify-content-between mt-2">
            <div>
                ...
                {{$pack->Product()->first()->name}} x
                <input min="1" class="pack text-center border-0" id="{{$pack->id}}" style="width: 3em" type="number"  value="{{$pack->quantity}}" required>
            </div>
            <div>  {{number_format($pack->Product()->first()->price*$pack->quantity)}} đ <a href="#" class="text-main"><i class="fas fa-trash-alt"></i> </a></div>
        </div>
        <hr>
    @endforeach
@else
